...
def upsmaple(inplanes, out_channels=8, kernel_size=7, stride=5, padding=1, bias=True):
    ...
    ret = deconv2_act(inplanes, out_channels = out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias)
    return ret
...
def mtest(num_seqs, channels_img, size_image, max_epoch, model, cuda_test):
    input_image = torch.rand(num_seqs, 384, channels_img, size_image, size_image)
    if cuda_test == True:
